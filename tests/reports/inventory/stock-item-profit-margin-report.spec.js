...
test.describe('Reports > Inventory > Stock Item Profit Margin', () => {

  test('[Happy Path] views, previews, and prints the Stock Item Profit Margin report', async ({ page }) => {
    test.setTimeout(90000);
    const inventoryReportPage = new InventoryReportPage(page);
    await inventoryReportPage.goto('Stock Item Profit Margin');

    await inventoryReportPage.selectFirstWarehouseIfNeeded();
    await inventoryReportPage.viewGrid();
    await inventoryReportPage.previewReport('Stock Item Profit Margin');
    ...
    await reportPage.screenshot({ path: 'test-results/stock-item-profit-margin-report-print.png', fullPage: true }).catch(() => {});
