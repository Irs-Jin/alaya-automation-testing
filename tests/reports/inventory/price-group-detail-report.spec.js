...
test.describe('Reports > Inventory > Price Group Detail', () => {

  test('[Happy Path] views, previews, and prints the Price Group Detail report', async ({ page }) => {
    test.setTimeout(90000);
    const inventoryReportPage = new InventoryReportPage(page);
    await inventoryReportPage.goto('Price Group Detail');

    await inventoryReportPage.selectFirstWarehouseIfNeeded();
    await inventoryReportPage.viewGrid();
    await inventoryReportPage.previewReport('Price Group Detail');
    ...
    await reportPage.screenshot({ path: 'test-results/price-group-detail-report-print.png', fullPage: true }).catch(() => {});
