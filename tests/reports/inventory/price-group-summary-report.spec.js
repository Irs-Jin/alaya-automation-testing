...
test.describe('Reports > Inventory > Price Group Summary', () => {

  test('[Happy Path] views, previews, and prints the Price Group Summary report', async ({ page }) => {
    test.setTimeout(90000);
    const inventoryReportPage = new InventoryReportPage(page);
    await inventoryReportPage.goto('Price Group Summary');

    await inventoryReportPage.selectFirstWarehouseIfNeeded();
    await inventoryReportPage.viewGrid();
    await inventoryReportPage.previewReport('Price Group Summary');
    ...
    await reportPage.screenshot({ path: 'test-results/price-group-summary-report-print.png', fullPage: true }).catch(() => {});
