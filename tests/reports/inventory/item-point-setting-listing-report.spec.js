...
test.describe('Reports > Inventory > Item Point Setting Listing', () => {

  test('[Happy Path] views, previews, and prints the Item Point Setting Listing report', async ({ page }) => {
    test.setTimeout(90000);
    const inventoryReportPage = new InventoryReportPage(page);
    await inventoryReportPage.goto('Item Point Setting Listing');

    await inventoryReportPage.selectFirstWarehouseIfNeeded();
    await inventoryReportPage.viewGrid();
    await inventoryReportPage.previewReport('Item Point Setting Listing');
    ...
    await reportPage.screenshot({ path: 'test-results/item-point-setting-listing-report-print.png', fullPage: true }).catch(() => {});
