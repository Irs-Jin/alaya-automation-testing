...
test.describe('Reports > Inventory > Outstanding Stock Transfer Request', () => {

  test('[Happy Path] views, previews, and prints the Outstanding Stock Transfer Request report', async ({ page }) => {
    test.setTimeout(90000);
    const inventoryReportPage = new InventoryReportPage(page);
    await inventoryReportPage.goto('Outstanding Stock Transfer Request');

    await inventoryReportPage.selectFirstWarehouseIfNeeded();
    await inventoryReportPage.viewGrid();
    await inventoryReportPage.previewReport('Outstanding Stock Transfer Request');
    ...
    await reportPage.screenshot({ path: 'test-results/outstanding-stock-transfer-request-report-print.png', fullPage: true }).catch(() => {});
