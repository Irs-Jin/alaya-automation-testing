...
test.describe('Reports > Inventory > Stock Transfer Listing', () => {

  test('[Happy Path] views, previews, and prints the Stock Transfer Listing report', async ({ page }) => {
    test.setTimeout(90000);
    const inventoryReportPage = new InventoryReportPage(page);
    await inventoryReportPage.goto('Stock Transfer Listing');
    ...
    await inventoryReportPage.previewReport('Stock Transfer Listing');
    ...
    await reportPage.screenshot({ path: 'test-results/stock-transfer-listing-report-print.png', fullPage: true }).catch(() => {});
