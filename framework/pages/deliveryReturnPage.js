...
/**
 * Page object for Sales > Delivery Return.
 *
 ...
 * DeliveryOrderPage/CloseSalesOrderPage: New -> select Customer -> Copy
 * From -> "Transfer from DO" dialog (Full Transfer tab, checkbox-select one
 * row, OK) -> Sales Branch + Warehouse + Reference No + Items all auto-fill
 * -> Post. Internal control root "DeliveryReturnDetail"/"cbpDeliveryReturn",
 * document prefix "DR-".
 *
 * CONFIRMED live, things worth flagging:
 * 1. Customer MUST be selected BEFORE clicking Copy From — confirmed live
 *    via a genuine "Please insert Customer." error dialog when attempted
 *    the other order. This is the reverse of Delivery Order/Close Sales
 *    Order, where the order didn't matter as much; here it's a hard
 *    requirement enforced by the app itself.
 * 2. Copy From opens a MENU (not a direct dialog like Delivery Order's
 *    single-source case) — "Delivery Order" is one item in it. The LI
 *    button click and the submenu item click both needed a direct DOM
 *    `.evaluate(el => el.click())` rather than a real Playwright click —
 *    same confirmed-necessary technique as Close Sales Order's own Copy
 *    From submenu (neither a plain click nor a hover made the submenu item
 *    genuinely visible/actionable per Playwright's own checks, even though
 *    the direct DOM click reaches the real handler and works).
 * 3. The New-record insert icon has an EMPTY `alt` attribute here (unlike
 *    every other screen in this module, where the same icon's alt text —
 *    "Click Here Or Press [Insert] To Add Record" — is what accessible-name
 *    role matching relies on). Must select it by `title` attribute via a
 *    plain CSS locator instead of `getByRole('img', {name: ...})`.
 * 4. No Delivery Address requirement (unlike Delivery Order/Goods Return) —
 *    confirmed live: Post succeeds right after the transfer with no
 *    address fields touched at all.
 * 5. "Save Draft" is not available once a Transfer From has completed (same
 *    pattern already confirmed for Goods Return/Close Sales Order/Delivery
 *    Order) — Post is the only way to persist a real document here.
 * 6. Post does NOT auto-open a report tab (confirmed live: just a generic
 *    "Saved Successfully" banner, same shape as Close Sales Order) — a
 *    "Preview Report" toolbar item appears afterward instead, but isn't
 *    needed for verification. Success/document-number are both read
 *    directly from the Document No. field (`txtDocNo_I`), matching Close
 *    Sales Order's own convention for report-less screens.
 * 7. The transferred document's own Reference No. (the original
 *    Quotation's reference, carried through Quotation -> Delivery Order ->
 *    Delivery Return) survives the whole chain unmodified — confirmed live
 *    — so no explicit Reference No. fill is needed here; the upstream
 *    Quotation's `TESTING-SQ-<timestamp>` reference is already a unique,
 *    searchable key for the Cancel test.
 *
...
class DeliveryReturnPage {
  ...
  async goto() {
    ...
    const link = navBar.getByRole('link', { name: 'Delivery Return', exact: true });
  ...
  async _resolveListFrame() {
    ...
    if (!this.listFrame) {
      await p.screenshot({ path: 'test-results/debug-delivery-return-list-page.png', fullPage: true }).catch(() => {});
      throw new Error(
        'Could not find the Delivery Return listing frame. ' +
        'Saved test-results/debug-delivery-return-list-page.png for inspection.'
      );
    }
  }
  ...
  async _resolveFormFrame() {
    ...
    if (!this.formFrame) {
      await p.screenshot({ path: 'test-results/debug-delivery-return-form-not-found.png', fullPage: true }).catch(() => {});
      throw new Error(
        'Could not find the Delivery Return create/edit form frame. ' +
        'Saved test-results/debug-delivery-return-form-not-found.png for inspection.'
      );
    }
  }

  /** CONFIRMED live: this icon's `alt` is empty here — must match by `title`, not accessible name/role. */
  async clickNew() {
    const addIcon = this.listFrame.locator('[title="Click Here Or Press [Insert] To Add Record" i]');
    await addIcon.first().waitFor({ state: 'visible', timeout: 5000 });
  ...
  async selectCustomer(customerCode) {
    ...
    const { locator: trigger } = await heal(f, {
      id: 'deliveryReturn.customerTrigger',
      label: 'Customer',
      strategies: [
    ...
    const { locator: okButton } = await heal(popupFrame, {
      id: 'deliveryReturn.customerPopupOkButton',
      label: 'OK',
      strategies: [
   ...
   * Customer must already be selected (a "Please insert Customer." error
   * dialog appears otherwise), and BOTH clicks need a direct DOM
   * `.evaluate(el => el.click())` — same confirmed-necessary technique as
   * Close Sales Order's own Copy From submenu.
   */
  async clickCopyFromDeliveryOrder() {
    const f = this.formFrame;
    const { locator: copyFromButton } = await heal(f, {
      id: 'deliveryReturn.copyFromButton',
      label: 'Copy From',
      strategies: [
  ...
  async selectDeliveryOrderToTransfer(doDocNo) {
    ...
    const { locator: okButton } = await heal(f, {
      id: 'deliveryReturn.transferOkButton',
      label: 'OK',
      strategies: [
  ...
  async prepareDeliveryReturn({ customerCode, doDocNo }) {
  ...
  /** CONFIRMED live: no auto-opened report tab — just waits for the Document No. field to move off its pre-save placeholder. */
  async clickPost() {
    const { locator } = await heal(this.formFrame, {
      id: 'deliveryReturn.postButton',
      label: 'Post',
      strategies: [
  ...
  async clickPostAndNew() {
    const { locator } = await heal(this.formFrame, {
      id: 'deliveryReturn.postAndNewButton',
      label: 'Post & New',
      strategies: [
  ...
  /** CONFIRMED live: no auto-opened report tab for this screen — reads the Document No. field directly, same convention as CloseSalesOrderPage. */
  ...
  async searchListing(searchText) {
    ...
    const { locator: filterBox } = await heal(this.listFrame, {
      id: 'deliveryReturn.listingSearchFilterBox',
      label: 'Search',
      strategies: [
    ...
    if (!matched) {
      await this.page.screenshot({
        path: `test-results/debug-delivery-return-search-not-found-${Date.now()}.png`,
        fullPage: true,
      }).catch(() => {});
      throw new Error(
        `Delivery Return listing never showed a row matching "${searchText}" after searching — ` +
  ...
  async confirmCancelYes() {
    ...
    const { locator: yesButton } = await heal(this.listFrame, {
      id: 'deliveryReturn.cancelConfirmYesButton',
      label: 'Yes',
      strategies: [
  ...
  async handleCancelReasonIfPresent() {
    ...
    const { locator: reasonTrigger } = await heal(this.listFrame, {
      id: 'deliveryReturn.cancelReasonTrigger',
      label: 'Reason',
      strategies: [
    ...
    const { locator: okButton } = await heal(this.listFrame, {
      id: 'deliveryReturn.cancelReasonOkButton',
      label: 'OK',
      strategies: [
...
module.exports = { DeliveryReturnPage };
