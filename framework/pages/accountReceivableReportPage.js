...
class AccountReceivableReportPage {
  ...
  async goto(reportName = 'Customer Outstanding') {
    ...
    // Scoped to #sub13 to disambiguate from any other "Account
    // Receivable" link elsewhere in the nav — this is the CATEGORY link,
    // same pattern as General Ledger's and Account Payable's.
    await p.locator('#sub13').getByRole('link', { name: 'Account Receivable', exact: true }).click();
