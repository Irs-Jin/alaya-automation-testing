...
class PurchaseReportPage {
  ...
  async goto(reportName) {
    ...
    // Scoped to #sub13 to disambiguate from any other "Purchase" link
    // elsewhere in the nav (e.g. the top-level Purchase MODULE, distinct
    // from the Reports > Purchase CATEGORY).
    await p.locator('#sub13').getByRole('link', { name: 'Purchase', exact: true }).click();
