...
/**
 * Page object for the "Reports > Membership Voucher" CATEGORY (the
 ...
 * `inventoryReportPage.js` / `purchaseReportPage.js` / `salesReportPage.js`
 * / `gstReportPage.js` (the SAME underlying
 * `ReportFrameWork1` reports framework, just a different catalog and
 * category link). Ninth category to reuse this exact shape. Only one
 * catalog entry so far: Voucher Utilization.
 *
 ...
 */
class MembershipVoucherReportPage {
  constructor(page) {
    this.page = page;
  ...
  async goto(reportName) {
    ...
    // Scoped to #sub13 to disambiguate from any other "Membership
    // Voucher" link elsewhere in the nav (e.g. the top-level Membership
    // Voucher MODULE, distinct from the Reports > Membership Voucher
    // CATEGORY).
    await p.locator('#sub13').getByRole('link', { name: 'Membership Voucher', exact: true }).click();
    ...
    if (!this.catalogFrame) {
      await p.screenshot({ path: 'test-results/debug-membershipvoucher-report-catalog-not-found.png', fullPage: true }).catch(() => {});
      throw new Error(
        `Could not find "${reportName}" in the report catalog after clicking Reports > Membership Voucher. ` +
        'Saved test-results/debug-membershipvoucher-report-catalog-not-found.png for inspection.'
      );
    }
    ...
    if (!this.reportFrame) {
      await p.screenshot({ path: 'test-results/debug-membershipvoucher-report-runtime-not-found.png', fullPage: true }).catch(() => {});
      throw new Error(
        `Could not find the report runtime frame after clicking into the "${reportName}" report. ` +
        'Saved test-results/debug-membershipvoucher-report-runtime-not-found.png for inspection.'
      );
    }
  }
  ...
  async _selectFirstRowInFilterBySelection(glFieldName) {
    ...
    const { locator } = await heal(f, {
      id: `membershipVoucherReport.${glFieldName.toLowerCase()}SelectionDropdownButton`,
    ...
    const { locator: firstRowCheckbox } = await heal(f, {
      id: `membershipVoucherReport.${glFieldName.toLowerCase()}SelectionFirstRow`,
  ...
  async viewGrid() {
    ...
    const { locator } = await heal(f, {
      id: 'membershipVoucherReport.viewGridButton',
      label: 'View Grid',
      strategies: [
  ...
  async previewReport(templateName) {
    ...
    const { locator: previewReportButton } = await heal(f, {
      id: 'membershipVoucherReport.previewReportButton',
    ...
    const { locator: previewButton } = await heal(f, {
      id: 'membershipVoucherReport.previewConfirmButton',
      label: 'Preview',
      strategies: [
  ...
  async printReport() {
    ...
    if (!printFrame) {
      await p.screenshot({ path: 'test-results/debug-membershipvoucher-report-print-button-not-found.png', fullPage: true }).catch(() => {});
      throw new Error(
        'Could not find the report\'s print button — the preview may not have finished rendering. ' +
        'Saved test-results/debug-membershipvoucher-report-print-button-not-found.png for inspection.'
      );
    }
...
module.exports = { MembershipVoucherReportPage };
