...
class InventoryReportPage {
  ...
  async goto(reportName) {
    ...
    // Scoped to #sub13 to disambiguate from any other "Inventory" link
    // elsewhere in the nav (e.g. the top-level Inventory MODULE, distinct
    // from the Reports > Inventory CATEGORY) — same pattern as General
    // Ledger's / Account Payable's / Account Receivable's category link.
    await p.locator('#sub13').getByRole('link', { name: 'Inventory', exact: true }).click();
