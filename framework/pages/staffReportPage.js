...
/**
 * Page object for the "Reports > Staff" CATEGORY (the #sub13 nav entry)
 ...
 * / `gstReportPage.js` / `membershipVoucherReportPage.js` (the SAME
 * underlying `ReportFrameWork1` reports framework, just a different
 * catalog and category link). Tenth category to reuse this exact shape.
 * Only one catalog entry so far: Sales Commission Listing.
 *
...
class StaffReportPage {
   ...
   *   "Sales Commission Listing").
   */
  async goto(reportName) {
    const p = this.page;

    await p.getByRole('link', { name: 'Reports', exact: true }).click();
    // Scoped to #sub13 to disambiguate from any other "Staff" link
    // elsewhere in the nav (e.g. the top-level Staff MODULE, distinct
    // from the Reports > Staff CATEGORY).
    await p.locator('#sub13').getByRole('link', { name: 'Staff', exact: true }).click();
    ...
    if (!this.catalogFrame) {
      await p.screenshot({ path: 'test-results/debug-staff-report-catalog-not-found.png', fullPage: true }).catch(() => {});
      throw new Error(
        `Could not find "${reportName}" in the report catalog after clicking Reports > Staff. ` +
        'Saved test-results/debug-staff-report-catalog-not-found.png for inspection.'
      );
    }
    ...
    if (!this.reportFrame) {
      await p.screenshot({ path: 'test-results/debug-staff-report-runtime-not-found.png', fullPage: true }).catch(() => {});
      throw new Error(
        `Could not find the report runtime frame after clicking into the "${reportName}" report. ` +
        'Saved test-results/debug-staff-report-runtime-not-found.png for inspection.'
      );
    }
  }
  ...
  async _selectFirstRowInFilterBySelection(glFieldName) {
    ...
    const { locator } = await heal(f, {
      id: `staffReport.${glFieldName.toLowerCase()}SelectionDropdownButton`,
    ...
    const { locator: firstRowCheckbox } = await heal(f, {
      id: `staffReport.${glFieldName.toLowerCase()}SelectionFirstRow`,
   ...
   * defaults to "ALL".
   *
   * @param {string} fieldName - e.g. "Customer" or "Staff".
   */
  ...
  async previewReport(templateName) {
    ...
    const { locator: previewReportButton } = await heal(f, {
      id: 'staffReport.previewReportButton',
    ...
    const { locator: previewButton } = await heal(f, {
      id: 'staffReport.previewConfirmButton',
      label: 'Preview',
      strategies: [
  ...
  async printReport() {
    ...
    if (!printFrame) {
      await p.screenshot({ path: 'test-results/debug-staff-report-print-button-not-found.png', fullPage: true }).catch(() => {});
      throw new Error(
        'Could not find the report\'s print button — the preview may not have finished rendering. ' +
        'Saved test-results/debug-staff-report-print-button-not-found.png for inspection.'
      );
    }
...
module.exports = { StaffReportPage };
