...
class OthersReportPage {
  ...
  async goto(reportName) {
    ...
    // Scoped to #sub13 to disambiguate from any other "Others" link
    // elsewhere in the nav (e.g. the top-level Others MODULE, distinct
    // from the Reports > Others CATEGORY).
    await p.locator('#sub13').getByRole('link', { name: 'Others', exact: true }).click();
